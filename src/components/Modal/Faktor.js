...
const ModalFaktor = ({isOpen, toggle, data, fetch, dataEdit}) => {
  ...
  const submit = async () => {
    try {
      ...
      let result;
      if (dataEdit) {
        result = await http(token).put(`api/v1/faktor/${dataEdit.id}`, payload);
      } else {
        result = await http(token).post('api/v1/faktor', payload);
      }
      await fetch();
      setLoading(false);
      toggle();
